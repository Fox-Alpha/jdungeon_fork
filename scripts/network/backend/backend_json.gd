class_name BackendJson extends BackendBase

# .env
# Only required if backend is "json"
# BACKEND_CONFIG="res://data/users.json"

func _init() -> void:
	backend_initialized = create_backend_if_not_exists(Global.env_json_backend_file, {})

## Create the initial Backend if it does not exists (maybe first start)
func create_backend_if_not_exists(path: String, json_data: Dictionary) -> bool:
	return create_file_if_not_exists(path, json_data)
